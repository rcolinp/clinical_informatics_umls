...
def extract_nodes_edges(db_dir: str, db_name: str):
	...
	semantic_node = """
					...
					WHERE c.SAB IN ('ATC', 'GO', 'HPO', 'ICD9CM', 'ICD10CM', 'NCI',
									'RXNORM', 'SNOMEDCT_US')
					AND c.SUPPRESS = 'N'
					AND c.LAT = 'ENG';
	"""
	...
	semanticTypeNode.to_csv(
		path_or_buf="../../../../import/semanticTypeNode.csv",
		header=True,
		index=False
	)
	...
	conceptNode.to_csv(
		path_or_buf="../../../../import/conceptNode.csv",
		header=True,
		index=False
	)
	...
	atom_node = """
				SELECT DISTINCT AUI, STR, SAB, CODE, TTY, ISPREF, TS, 'AUI' AS ":LABEL"
				FROM MRCONSO
				WHERE SAB IN ('ATC', 'GO', 'HPO', 'ICD9CM', 'ICD10CM', 'NCI',
							  'RXNORM', 'SNOMEDCT_US')
					AND SUPPRESS = 'N'
	...
	atomNode.columns = ["AUI:ID", "STR", "SAB", "CODE",
						"TTY", "ISPREF", "TS", ":LABEL"]
	...
	has_cui_rel.to_csv(
		path_or_buf="../../../../import/has_cui_rel.csv",
		header=True,
		index=False
	)
	...
	tui_tui_rel.to_csv(
		path_or_buf="../../../../import/tui_tui_rel.csv",
		header=True,
		index=False
	)
	...
	child_of_rel.to_csv(
		path_or_buf="../../../../import/child_of_rel.csv",
		header=True,
		index=False
	)
	...
	cui_code_rel = """
					SELECT DISTINCT CUI, (SAB || '#' || CODE) AS ":END_ID", 'HAS_SOURCE_CODE'
					FROM MRCONSO
					WHERE SAB IN ('ATC', 'GO', 'HPO', 'ICD9CM', 'ICD10CM', 'NCI',
								'RXNORM', 'SNOMEDCT_US')
						AND SUPPRESS = 'N'
	...
	cui_code_rel_icdo_append = icdo_df.merge(
		has_source_code_copy,
		how='inner',
		on=':END_ID'
	)

	cui_code_rel_icdo_append[':TYPE'] = 'HAS_SOURCE_CODE'
	cui_code_rel_append = cui_code_rel_icdo_append[[':START_ID', 'Code:ID', ':TYPE']].rename(
		{'Code:ID': ':END_ID'}, axis=1).drop_duplicates().replace(np.nan, '')
